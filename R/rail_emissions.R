...
rail_emissions <- function(from, to, via = NULL, num_people = 1, times_journey = 1, round_trip = FALSE){
  ...
  if (!(from) %in% c(stations$Name)){
    station_names <- agrep(data.frame(from), stations$Name, ignore.case = TRUE, max.distance = 0.15, value = TRUE)
    stop(print(from), " is not a name in the data frame. Did you mean: ",
         paste0((data.frame(stations) %>% dplyr::filter(Name %in% station_names))$Name, sep = ", ")
         )
  }
  if (!(to) %in% c(stations$Name)){
    station_names <- agrep(data.frame(to), stations$Name, ignore.case = TRUE, max.distance = 0.15, value = TRUE)
  ...
  if (!is.null(via)){
    ...
    via_x <- via[i]
    if (!(via_x) %in% c(stations$Name)){
      station_names <- agrep(data.frame(via_x), stations$Name, ignore.case = TRUE, max.distance = 0.15, value = TRUE)
  ...
  i <- which(stations$Name == {{ from }})
  j <- which(stations$Name == {{ to }})
  k <- stations[match({{ via }}, stations$Name), ]$id # to keep order
